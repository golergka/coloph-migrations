...
Use the project's pinned CLI through `uv run coloph-migrate` for migration operations. Read
...
migration and snapshot paths below.

Read the committed schema snapshot for current structure. Add the requested
change with `uv run coloph-migrate new NAME`. Use `uv run coloph-migrate check-chain` to check
...
Use `uv run coloph-migrate --json plan` to inspect pending migrations and checksum
...
Run `uv run coloph-migrate --json dry-run` before snapshot work. It executes the
selected chain in a disposable database and detects transaction escapes.

Regenerate with `uv run coloph-migrate snapshot --fresh`, review the resulting SQL,
...
Run `uv run coloph-migrate --json verify` after updating the migration and snapshot.
...
`uv run coloph-migrate --json check-backwards`. Inspect its status: `skipped` does not
...
Apply through the project's deployment workflow. Use `uv run coloph-migrate apply`
